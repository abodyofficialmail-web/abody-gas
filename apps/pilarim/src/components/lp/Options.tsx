  ...
  return (
    <section className="py-14 sm:py-16 bg-neutral-50">
      <div className="max-w-6xl mx-auto px-4 sm:px-6">
        <h2 className="text-xl sm:text-3xl font-bold text-center text-pilarim-ink mb-2 tracking-wide">
          オプション
        </h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {TICKET_PLANS.map((item) => (
            <div
              key={item.name}
              className="bg-white rounded-3xl p-6 shadow-soft border border-neutral-200 min-w-0"
            >
              ...
              <div className="mt-5 grid grid-cols-2 gap-3">
                {item.prices.map((price) => (
                  <div key={price.label} className="bg-pilarim-cream rounded-xl px-3 py-3">
                    ...
                    <p className="text-xl font-bold text-pilarim-bronze mt-1">
                      {price.value}
                      <span className="text-xs font-medium text-neutral-500 ml-1">円</span>
                    </p>
                    <p className="text-[10px] text-pilarim-bronze">税抜</p>
                    {price.note && (
                      <p className="text-[10px] font-bold text-pilarim-bronze mt-1">{price.note}</p>
                    )}
                  </div>
                ))}
              </div>
