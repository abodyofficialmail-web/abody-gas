...
import { INSTAGRAM_REELS, STUDIO_SCENES } from "@/lib/constants";
      ...
      {showThumb ? (
        <>
          ...
          <div className="absolute inset-0 flex items-center justify-center bg-black/20 group-hover:bg-black/30 transition-colors">
            <span className="w-14 h-14 rounded-full bg-white/90 flex items-center justify-center text-2xl pl-1 text-neutral-800 shadow-lg">
              ▶
            </span>
          </div>
        </>
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6">
          ...
          <div className="flex gap-4 overflow-x-auto snap-x snap-mandatory scrollbar-hide -mx-4 px-4 pb-2">
            {STUDIO_SCENES.map((scene) => (
              <div
                key={scene.src}
                className="relative flex-shrink-0 w-[280px] sm:w-[320px] snap-start rounded-2xl overflow-hidden shadow-soft border border-neutral-100 aspect-[9/16] max-h-[400px] bg-neutral-100"
              >
                <Image
                  src={scene.src}
                  alt={scene.alt}
